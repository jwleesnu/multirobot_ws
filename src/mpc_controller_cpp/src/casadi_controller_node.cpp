...
public:
  CasadiMPCControllerNode() : rclcpp::Node("casadi_mpc_controller_node") {
    ...
    params_sys_r_BtoR_pairs_ = param_list_to_pairs(
      this->get_parameter("sys.r_BtoR").as_double_array(), 4
    );
...
private:
  ...
  void control_tick() {
    ...
    try {
      ...
      std::vector<double> thR_list(4, 0.0);
      for (int i = 0; i < 4; ++i) {
    ...
    for (int i = 0; i < 4; ++i) P[3+i] = thR_[i];
    // Xb_desr stacked
    int base = NX;
    for (int i = 0; i < N; ++i) {
      P[base + 3*i + 0] = xref[i];
    ...
    if (!x_prev_valid_) {
      ...
      x0.reserve(len_X_col_ + len_Up_col_ + len_U_col_);
      {
        ...
        std::vector<double> state0 = {xB_, yB_, thB_, thR_[0], thR_[1], thR_[2], thR_[3]};
        for (int col = 0; col < N+1; ++col) {
          x0.insert(x0.end(), state0.begin(), state0.end());
        }
      }
      {
        // Up0 (NUP x N), seed with [xB,yB,1e-3, 0,0,0,0]
        std::vector<double> up0 = {xB_, yB_, 1e-3, 0.0, 0.0, 0.0, 0.0};
